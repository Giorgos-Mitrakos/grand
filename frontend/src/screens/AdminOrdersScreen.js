              ...
              <tbody>
                {orders.map(order => (<tr key={order.order_id}>
                  ...
                  <td>{order.orderDate && Intl.DateTimeFormat('en-GB', {
                    ...
                    hour12: false
                  }).format(Date.parse(order.orderDate))}</td>
                  <td>{order.user_email}</td>
                  <td>{order.paymentMethod}</td>
                  <td>{order.paymentType}</td>
                  <td>{order.shippingDate && convertDate(order.shippingDate)}</td>
                  <td>{order.totalPrice} €</td>
                  <td>{order.status}</td>
                  <td>
                    <Link to={"/admin/order/" + order.order_id} className="order_info_button" >
                      <span class="material-icons" data-tip data-for="order_info">
                        info
                      </span>
                    </Link>
                    <ReactTooltip backgroundColor="#deccf0" textColor="#312f8b" id="order_info" place="top" effect="solid">
                      Λεπτομέριες
                    </ReactTooltip>
                  </td>
                </tr>))}
