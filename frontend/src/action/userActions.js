...
const listNewsletters = (itemsPerPage, offset) =>async (dispatch, getState) =>{    
    try{
        dispatch({type: NEWSLETTER_LIST_REQUEST, payload: {}});
        const {userSignin:{userInfo}} = getState();
        const {data} = await Axios.post("/api/admin/newsletterlist", {itemsPerPage, offset },
        {headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
export {signin, signout, register, saveAccountInfo, accountInfo, passwordChange,
    ...
    insertAdmin, deleteAdmin, listNewsletters}