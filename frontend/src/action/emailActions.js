...
const sendNewOrderEmail = (orderId) => async (dispatch)=>{
    try {
        dispatch({type:SEND_NEW_ORDER_EMAIL_REQUEST, payload:"sending"});
        await Axios.post("/api/email/sendNewOrderNotification", {orderId});
        dispatch({type: SEND_NEW_ORDER_EMAIL_SUCCESS, payload:"success"});
    } catch (error) {
        dispatch({type: SEND_NEW_ORDER_EMAIL_FAIL, payload: error.message});
    }
}
...
export {sendRegisterConfirmation, sendEmailToContact, sendNewsLetterMailConfirmation,
    ...
    updatePass,sendUpdatePassConfirm,sendAccountDeleteEmail,sendNewOrderEmail}