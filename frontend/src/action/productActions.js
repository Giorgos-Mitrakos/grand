...
const changeVisibility = (productID,productVisibility)=>async (dispatch,getState)=>{
    try{
        ...
        const {data}= await Axios.put("/api/admin/changeVisibility",{productID,productVisibility},{headers:{
...
const changeCategoryPercentage = (pricePercentage,category,subcategory)=> async(dispatch,getState)=>{
    try{
        const {userSignin:{userInfo}} = getState();
        dispatch({type:CATEGORY_PERCENTAGE_CHANGE_REQUEST});
        const {data}= await Axios.put("/api/admin/category_percentage_change",{pricePercentage, category,subcategory},{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const getProductsByCategoryAdmin = (category,subcategory) =>async (dispatch)=> {
    try{
        dispatch({type:PRODUCT_LIST_BY_CATEGORY_ADMIN_REQUEST});
        const {data}= await Axios.post("/api/products/products_by_category_admin",{category,subcategory});
...
const saveProduct = (productId,product) =>async (dispatch, getState) =>{
    try {
        ...
        if(!productId)
        {
            const {data} = await Axios.post("/api/admin/createproduct", product, {headers:{
                'Authorization': 'Bearer ' + userInfo.token
            }
            });
            ...
            const {data} = await Axios.put("/api/admin/createproduct/"+ productId, product, {headers:{
                'Authorization': 'Bearer ' + userInfo.token
            }
            });
            dispatch({type:PRODUCT_SAVE_SUCCESS, payload:data});
...
const listManufacturers = () => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/manufacturerslist",{} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const insertManufacturer = (manufacturer) => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/insertmanufacturer", {manufacturer} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const listSendingMethods = () => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/sendinglist",{} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const editSendingMethods = (methodId,method,cost) => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/editsendingmethod",{methodId,method,cost} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const createSendingMethods = (method,cost) => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/createsendingmethod",{method,cost} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const deleteSendingMethod = (methodId) => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/removesendingmethod",{methodId} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const listPaymentMethodsAdmin = (methodId) => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/paymentlist",{methodId} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const editPaymentMethods = (paymentMethodId,paymentMethod,paymentMethodCost,sendingMethodId) => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/editpaymentlist",{paymentMethodId,paymentMethod,paymentMethodCost,sendingMethodId} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const createPaymentMethod = (paymentMethod,paymentMethodCost,sendingMethodId) => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/createpaymentlist",{paymentMethod,paymentMethodCost,sendingMethodId} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const deletePaymentMethod = (paymentMethodId,sendingMethodId) => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/removepaymentlist",{paymentMethodId,sendingMethodId} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const listFeatureTitles = () => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/featuretitlelist",{} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const insertFeatureTitle = (featureTitle) => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/insertfeaturetitle", {featureTitle} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const getFeatureNames = (featureTitleId) =>async (dispatch, getState)=> {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/featurenames", {featureTitleId} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const insertFeatureName = (titleId, name) => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/insertfeaturename", {titleId,name} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const listFeatures = (productId) => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/featurelist",{productId} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const insertFeature = (productId, title, name) => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/insertfeature", {productId,title,name} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const deleteFeature = (productId,featureId) => async (dispatch, getState) => {
    try{        
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/deletefeature/",{productId,featureId}, {headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const listCategories = () => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/categories",[],{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const listSubcategories = (parentId) => async (dispatch, getState) => {
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/subcategories",{parentId},{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const listCompatibilityCompanies = () =>async (dispatch, getState)=>{
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/compatibilitycompanylist",{} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const getCompatibilityModels = (companyId) =>async (dispatch, getState)=>{
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/compatibilitymodelslist",{companyId} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const insertCompatibilityCompany = (company) =>async (dispatch, getState)=>{
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/insertcompatibilitycompany",{company} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const insertCompatibilityModel = (companyId,model) =>async (dispatch, getState)=>{
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/insertcompatibilitymodel",{companyId,model} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const getProductCompatibilities = (productId) =>async (dispatch, getState)=>{
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/getproductcompatibilities",{productId} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const insertCompatibility = (productId,company,model) =>async (dispatch, getState)=>{
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        const {data}= await Axios.post("/api/admin/insertcompatibility",{productId,company,model} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
const deleteProductCompatibility = (compatId) =>async (dispatch, getState)=>{
    try{
        ...
        const {userSignin:{userInfo}} = getState();
        await Axios.post("/api/admin/deletecompatibility",{compatId} ,{headers:{
            'Authorization': 'Bearer ' + userInfo.token
        }});
...
export {listProducts, detailsProduct, saveProduct,removeProducts, listSendingMethods,
